...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if(isset($_POST['login_btn'])) {
        $moderatorname = filter_var($_POST['log_user'], FILTER_SANITIZE_STRING);
        $_SESSION['log_user'] = $moderatorname;
        $password = $_POST['log_password'];

        // Query to fetch the moderator details from the database
        $check_database_query = mysqli_query($con, "SELECT * FROM moderator WHERE moderatorname='$moderatorname'") or die(mysqli_error($con));
        $check_login_query = mysqli_num_rows($check_database_query);

        if ($check_login_query == 1) {
            $moderator = mysqli_fetch_array($check_database_query) or die(mysqli_error($con));
            if (password_verify($password, $moderator['password'])) {
                $_SESSION['moderator_logged_in'] = true;
                $_SESSION['moderatorname'] = $moderator['moderatorname'];
